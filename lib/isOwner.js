...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    let configuredOwners = [settings.ownerNumber];
    try { configuredOwners = JSON.parse(fs.readFileSync('./data/owners.json', 'utf8')); } catch (_) {}
    const ownerJid = settings.ownerNumber + "@s.whatsapp.net";
    const ownerNumberClean = settings.ownerNumber.split(':')[0].split('@')[0];
    
    const senderIdClean = senderId.split(':')[0].split('@')[0];
    if (configuredOwners.map(String).includes(senderIdClean)) return true;
