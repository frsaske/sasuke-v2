...
// Rotation list for auto-reactions. One emoji is selected per incoming message.
const commandEmojis = ['😍', '💔', '😭', '🥀', '😹', '🤣', '💅', '❤‍🔥', '👁', '😇', '😂', '😒', '🥰', '😜'];
...
async function addCommandReaction(sock, message) {
    try {
        if (!isAutoReactionEnabled || !message?.key?.id || message.key.fromMe || message.key.remoteJid === 'status@broadcast') return;
        sock.__autoReactedMessageIds = sock.__autoReactedMessageIds || new Set();
        if (sock.__autoReactedMessageIds.has(message.key.id)) return;
        sock.__autoReactedMessageIds.add(message.key.id);
        if (sock.__autoReactedMessageIds.size > 500) sock.__autoReactedMessageIds.delete(sock.__autoReactedMessageIds.values().next().value);
